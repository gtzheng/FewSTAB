...
class AbstractModel(nn.Module):
    ...
    def _convert_global_to_local(self, global_target):
        num_episodes, num_ways, num_shots = global_target.shape
        local_labels = global_target.clone().detach()
        for e in range(num_episodes):
            labels = global_target[e][:, 0]
            label2idx = {l.item(): i for i, l in enumerate(labels)}
            func = lambda x: label2idx[x]
            local_labels[e].apply_(func)
        return local_labels

    def split_by_episode(self, features, global_target, mode):
        """
        ...
        if getattr(self, "worst_group", False):
            local_labels = self._convert_global_to_local(global_target).to(self.device)
        else:
            local_labels = (
