...
class Trainer(object):
    ...
    def _init_dataloader(self, config):
        ...
        spurious_mode = config.get("spurious", "offline")
        train_loader = get_dataloader(
            config, "train", self.model_type, distribute, config.get("spurious", False)
        )
        val_loader = get_dataloader(
            config, "val", self.model_type, distribute, config.get("spurious", False)
        )
        test_loader = get_dataloader(
            config, "test", self.model_type, distribute, config.get("spurious", False)
        )
    ...
    def _init_optim(self, config):
        ...
        if self.config["resume"]:
            ...
            best_test_acc = all_state_dict[
                "best_val_acc"
            ]  # FIXME: should be best_test_acc
