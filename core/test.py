...
def get_images(data, config, nrow=5):
    import matplotlib.pyplot as plt
    import torchvision
    from core.data.collates import get_mean_std

    MEAN, STD = get_mean_std(config, "test")
    ori_data = data * (torch.tensor(STD).view(1, 3, 1, 1)) + torch.tensor(MEAN).view(
        1, 3, 1, 1
    )
    grids = torchvision.utils.make_grid(ori_data, nrow=nrow)
    fig, ax = plt.subplots()
    ax.imshow(grids.detach().permute(1, 2, 0).cpu().numpy())
...
class Test(object):
    ...
    def __init__(self, rank, config, result_path=None):
        ...
        self.model.worst_group = config.get("worst_group", False)
    ...
    def test_loop(self):
        ...
        with open(self.config["result_path"], "a") as f:
            f.write(
                f"{self.config['classifier']['name']}, {self.config['backbone']['name']}, train_shot:{self.config['train_shot']}, acc:{aver_accuracy:.4f}±{h:.4f}\n"
            )
    ...
    def _validate(self, epoch_idx):
        ...
        with torch.set_grad_enabled(enable_grad):
            ...
            for batch_idx, batch in enumerate(zip(*loader)):
                ...
                calc_begin = time()
                if self.config.get("worst_group", False):
                    acc = 100.0
                    num_per_task = self.config["test_way"] * (
                        self.config["test_shot"] + self.config["test_query"]
                    )

                    for c in range(self.config["test_way"]):
                        output, acc_cls = self.model(
                            [
                                batch[0][0][c * num_per_task : (c + 1) * num_per_task],
                                batch[0][1][c : c + 1],
                            ]
                        )
                        if acc_cls < acc:
    ...
    def _init_dataloader(self, config):
        ...
        test_loader = get_dataloader(
            config, "test", self.model_type, distribute, config["spurious"]
        )
