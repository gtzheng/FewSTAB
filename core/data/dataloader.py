...
import pickle
from spurious.spurious_task import SpuriousTask, SpuriousSampler


def get_dataloader(config, mode, model_type, distribute, spurious=None):
    ...
    if config["dataloader_num"] == 1 or mode in ["val", "test"]:
        ...
        if spurious:
            # assert mode in ["val", "test"], "must in val/test mode"
            if config.get("attribute_path_train", None) is None:
                raise ValueError(("attribute_path_train not provided"))
            attr_task_obj = SpuriousTask(
                config["attribute_path_test"]
                if mode == "test"
                else config["attribute_path_val"],
                config["test_episode"],
                config["test_way"],
                config["test_shot"],
                config["test_query"],
            ...
            sampler = SpuriousSampler(
                attr_task_obj,
                config["task_path"],
                config["test_episode"] // config["episode_size"],
                config["test_way"],
                config["test_shot"],
                config["test_query"],
                ep_per_batch=config["episode_size"],
                mode=config["spurious_mode"],
                worst_group=config.get("worst_group", True),
            )
        else:
            sampler = get_sampler(
                dataset=dataset,
