    ...
    if few_shot:
        if distribute:
            sampler = DistributedCategoriesSampler(
                ...
                shot_num=config["shot_num"] if mode == "train" else config["test_shot"],
                ...
                worst_group=False
                if mode == "train"
                else config.get("worst_group", False),
            )
        else:
...
class CategoriesSampler(Sampler):
    ...
    def __init__(
        ...
        Args:
            ...
            episode_size (int): The number of episodes per batch.
        ...
        if worst_group:
            assert episode_size == 1, "episode_size == 1"
            ...
            if len(batch) == self.episode_size * self.way_num:
                if self.worst_group:
                    ...
                    for c in range(self.way_num):
                        cls_batch = torch.cat(
                            (
                                batch[:, 0 : self.shot_num],
                                batch[c][self.shot_num :].repeat(self.way_num, 1),
                            ),
                            dim=1,
                        )
                        new_batch.append(cls_batch)
                    batch = torch.stack(new_batch).reshape(-1)
                    yield batch
                else:
